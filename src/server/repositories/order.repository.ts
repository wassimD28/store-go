...
export class OrderRepository {
  ...
  static async create(orderData: OrderCreateData) {
    try {
      ...
      const [newOrder] = await db.insert(AppOrder).values({
  ...
  static async update(id: string, orderData: OrderUpdateData) {
    try {
      ...
      const updateValues = {
        ...
        ...(orderData.payment_status && { payment_status: orderData.payment_status }),
        ...(orderData.address_id !== undefined && { address_id: orderData.address_id }),
        ...(orderData.paymentId !== undefined && { paymentId: orderData.paymentId }),
        ...(orderData.data_amount !== undefined && { data_amount: String(orderData.data_amount) })
      };
  ...
  static async findByPaymentStatus(userId: string, paymentStatus: string) {
    try {
      ...
      console.error(`Failed to fetch orders with payment status ${paymentStatus}:`, error);
      throw error;
    }
  }
}